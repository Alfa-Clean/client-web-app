 ...
 *
 * Раньше это была защита: воркер проставлял `X-Service-Key` на всё, и запрос
 * без токена уходил от имени сервиса, возвращая **чужие** заказы. Ключ из
 * воркера убран, такой запрос теперь получает честный 401 — но ходить за ним
 * незачем: пока клиент не опознан, своих заказов у него нет.
 */
